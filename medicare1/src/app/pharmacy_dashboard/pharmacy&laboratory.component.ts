...
export class PharmacyComponent implements OnInit {
  ...
  filteredPayments: Payment[] = [];  // Modal properties
  ...
  // New prescription modal properties
  isNewPrescription: boolean = false;
  ...
  onSavePrescription(prescription: Prescription): void {
    if (this.isNewPrescription) {
      ...
      console.log('New prescription added:', prescription);
    } else {
      // Update existing prescription
      const index = this.recentPrescriptions.findIndex(p => p.id === prescription.id);
      if (index !== -1) {
        this.recentPrescriptions[index] = prescription;
  ...
  onSaveLabTest(labTest: LabTest): void {
    if (this.isNewPrescription) { // Using same flag for new items
      // Add new lab test
      this.labTests.unshift(labTest);
      console.log('New lab test added:', labTest);
    } else {
      // Update existing lab test
      const index = this.labTests.findIndex(t => t.id === labTest.id);
      if (index !== -1) {
        this.labTests[index] = labTest;
        console.log('Lab test updated:', labTest);
      }
    }
  ...
  onSaveInventoryItem(inventoryItem: InventoryItem): void {
    if (this.isNewPrescription) { // Using same flag for new items
      // Add new inventory item
      this.inventoryItems.unshift(inventoryItem);
      console.log('New inventory item added:', inventoryItem);
    } else {
      // Update existing inventory item
      const index = this.inventoryItems.findIndex(i => i.id === inventoryItem.id);
      if (index !== -1) {
        this.inventoryItems[index] = inventoryItem;
        console.log('Inventory item updated:', inventoryItem);
      }
    }
  ...
  addNewLabTest(): void {
    console.log('Opening new lab test modal...');
    this.modalType = 'labtest';
    this.isNewPrescription = true; // Using same flag for both new items
    this.selectedPrescription = null;
    this.selectedLabTest = null;
    this.isModalVisible = true;
  }
  addNewInventoryItem(): void {
    ...
    this.isNewPrescription = true; // Using same flag for all new items
    this.selectedPrescription = null;
    this.selectedLabTest = null;
    this.selectedInventoryItem = null;
    this.isModalVisible = true;
